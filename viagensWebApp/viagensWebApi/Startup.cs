...
using ViagensWebApi.Services;
using Microsoft.EntityFrameworkCore;
using Pomelo.EntityFrameworkCore.MySql;

namespace ViagensWebApi
{
    public class Startup : IStartup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddDbContext<AppDbContext>(options =>
                {
                    var connetionString = Configuration.GetConnectionString("Conectar");
                    options.UseMySql(connetionString, ServerVersion.AutoDetect(connetionString));
                });
            //options.UseMySql(Configuration.GetConnectionString("Conectar")));

            services.AddScoped<IDestinoService, DestinosServiceBase>();
